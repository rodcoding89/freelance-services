...
const InvoiceForm:React.FC<InvoiceFormProps> = ({locale}) =>{
  ...
  const generatePdf = async (data : FormValues,logoUrl:string ) => {
    if (!process.env.NEXT_PUBLIC_COMPANY_NAME || !process.env.NEXT_PUBLIC_COMPANY_STREET || !process.env.NEXT_PUBLIC_COMPANY_POSTAL_CODE || !process.env.NEXT_PUBLIC_COMPANY_CITY || !process.env.NEXT_PUBLIC_COMPANY_COUNTRY || !process.env.NEXT_PUBLIC_WEB_LINK || !process.env.NEXT_PUBLIC_TAX_ID || !process.env.NEXT_PUBLIC_ROOT_LINK || !clientInfo) return 
