...
const InvoiceForm:React.FC<InvoiceFormProps> = ({locale}) =>{
  ...
  const generatePdf = async (data : FormValues,logoUrl:string ) => {
    if (!process.env.NEXT_PUBLIC_COMPANY_NAME || !process.env.NEXT_PUBLIC_COMPANY_STREET || !process.env.NEXT_PUBLIC_COMPANY_POSTAL_CODE || !process.env.NEXT_PUBLIC_COMPANY_CITY || !process.env.NEXT_PUBLIC_COMPANY_COUNTRY || !process.env.NEXT_PUBLIC_WEB_LINK || !process.env.NEXT_PUBLIC_TAX_ID || !process.env.NEXT_PUBLIC_ROOT_LINK) return 
    ...
    const pdfContent:pdfContent = {
      ...
      4:{
        isConditional:false,
        params:[[invoiceInfoX + 20,-15,18,helveticaBoldFont,`${t.invoice.billNr} ${data.invoiceInfo.number}`,page,whiteColor,50,yPositionLeft],[invoiceInfoX,20,11,helveticaBoldFont,t.invoice.billDate,page,blackColor,0,yPositionLeft],[invoiceInfoX + calculateAccurateTextWidth(t.invoice.billDate, helveticaBoldFont, 11),20,10,helveticaFont,` ${formatDate(data.invoiceInfo.date,data.locale)}`,page,blackColor,18,yPositionLeft],[invoiceInfoX,20,11,helveticaBoldFont,t.invoice.customerNr,page,blackColor,0,yPositionLeft],[invoiceInfoX + calculateAccurateTextWidth(t.invoice.customerNr, helveticaBoldFont, 11),20,10,helveticaFont,` ${data.clientInfo.clientNumber}`,page,blackColor,18,yPositionLeft]]
      },
      5:{
        isConditional:true,
  ...
  const handleSubmit = async(e: React.FormEvent) => {
    ...
    const data = {clientInfo,invoiceInfo,features,taxEnabled,taxRate,discount,invoiceDescription} as FormValues
    ...
    async function getDocumentById(collectionName: string, id: string,serviceId:string) {
      ...
      if (clientSnap.exists() && serviceSnap.exists()) {
        ...
        setClientInfo({
          ...
          contractStatus : service.contractStatus,
          contractType : service.serviceType,
        ...
        if (contract) {
          ...
          setInvoiceInfo((prev)=> {
            return {...prev,adresse:contract.clientAddress}
          })
  ...
  if (!clientInfo) return <div className="text-center py-8 mt-[110px] h-[200px] flex justify-center items-center w-[85%] mx-auto">Chargement...</div>;
  return (
    <div className="max-w-4xl mx-auto p-6 bg-white rounded-lg shadow-md mt-[110px] w-[85%]">
      <h1 className="text-2xl font-bold text-primary mb-6">Facturation pour le client {clientInfo.name}</h1>
